...
显式配置的 `backup.sources`（目录或文件）均为必需来源，不会静默忽略缺失项。来源收集或文件读取失败时，备份返回错误，不发布不完整归档，也不触发旧备份保留清理；HTTP 创建接口返回失败而非 201。只有完整归档发布成功后才执行 `max_backups` 清理。默认自动发现目录仍仅包含当前存在的默认目录。备份读取通过来源根目录句柄进行，只接受普通文件；根目录内的相对文件链接可读取，越界链接会使整次备份失败并保留旧归档。tar 条目统一使用 `/`，便于跨平台恢复；这不提供活跃数据库的一致性快照保证。
