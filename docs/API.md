...
限流决策、消费后的剩余额度和下一次额度释放时间在同一锁内计算。`X-RateLimit-Reset` 保持 Unix 毫秒单位；`429` 的 `Retry-After` 与错误详情 `retry_after` 使用同一正整数秒数，按决策时剩余等待向上取整。过期记录不影响重置时间，恰好到期的记录不再占用额度。响应描述本次决策的快照，并非客户端收到响应时的实时余额。
