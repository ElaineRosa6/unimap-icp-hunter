...
最新代码发布、运行实例、扩展源码与实际加载的区别见 [发布快照最新核验节](CURRENT_STATUS_2026-09-06.md#最新核验代码发布与运行实例分列)。62302b2已通过CI；本机新程序部署未执行，Bridge离线。下面的8月矩阵与已完成标签只代表原日期，不证明最新七引擎CDP、插件完整字段/分页或云端证据截图验收完成。原有明确暂缓的功能仍保持暂缓。
